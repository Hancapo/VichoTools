import bpy
from ..vicho_panels import VichoFragmentToolsPanel
...
class Vicho_FragmentGlassWindowsPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        list_col2.operator("glass_frag_list.remove_glass", icon="REMOVE", text="")
        list_col.separator()
        list_col.operator("glass_frag_list.export_xml", icon="EXPORT", text="Export to XML")
        list_col.separator()
        ...
        if 0 <= active_index < len(scene.glass_frag_list):
            ...
            layout.label(text="Glass Window Properties", icon="MOD_TRIANGULATE")
